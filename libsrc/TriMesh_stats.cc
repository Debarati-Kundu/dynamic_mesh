...
void TriMesh::write_vertex_outfile()
{
	FILE *fp;
	long int nv, i, j;
	long int sum = 0;
	dprintf("\nThis is a stub");
	fp = fopen("adjacency","w");
	need_neighbors();
	nv = vertices.size();	
	for (i = 0; i < nv; i++) {
...
void TriMesh::display_partition()
{
	int i,j, npart = 15, rand_col[15][3];
	int nv = vertices.size();
	colors.resize(nv);
	std::vector<int> part_count;
	part_count.resize(npart);
	for (i = 0; i < npart; i++)
		part_count[i] = 0;
	for (i = 0; i < nv; i++) 
		part_count[partition[i]]++;
	
	for (i = 0; i < npart; i++) {
		for (j = 0; j < 3; j++) {
			rand_col[i][j] = rand() % 256;
		}
	}

	  for (i = 0; i < nv; i++) {
			//dprintf("\ninside");
                          Color cc = Color(rand_col[partition[i]][0], rand_col[partition[i]][1], rand_col[partition[i]][2]);
...
/*	
	...
	for (i = 0; i < npart; i++)
		dprintf("\n%d %d %d",rand_col[i][0], rand_col[i][1], rand_col[i][2]);
*/	
	dprintf("\nLast line\n");
	return;
}
