...
double euclid_distance(point v1, point v2)
{
	point diff = v1 - v2;
	return sqrt(diff[0]*diff[0] + diff[1]*diff[1] + diff[2]*diff[2]);
}

bool TriMesh::is_vertex_in_face(int m, int n)
{
	if (m == faces[n][0] || m == faces[n][1] || m == faces[n][2])
		return true;
	return false;
}

void TriMesh::laplace_beltrami()
{
	int i, j, k, n;
	int nv = vertices.size();
	int ind_faces[2][2];
	double** lb_operator_A = new double*[nv];
	double** lb_operator_B = new double*[nv];
	for(i = 0; i < nv; i++) {
    		lb_operator_A[i] = new double[nv];
		lb_operator_B[i] = new double[nv];
	}
	need_neighbors();
	need_adjacentfaces();
	//for (i = 0; i < nv; i++) {
		...
		for (j = 0; j < neighbors[i].size(); j++) {
			n = adjacentfaces[i].size();
			ind_faces[0][0] = -1;
			ind_faces[0][1] = -1;
			ind_faces[1][0] = -1;
			ind_faces[1][1] = -1;
			int ind_faces_count = 0;
			for (k = 0; k < n; k++) {
				if(is_vertex_in_face(neighbors[i][j],adjacentfaces[i][k])) {
					ind_faces[ind_faces_count][0] = adjacentfaces[i][k];
					if (faces[adjacentfaces[i][k]][0] == neighbors[i][j])
						ind_faces[ind_faces_count][1] = faces[adjacentfaces[i][k]][0];
					if (faces[adjacentfaces[i][k]][1] == neighbors[i][j])
                                                ind_faces[ind_faces_count][1] = faces[adjacentfaces[i][k]][1];
					if (faces[adjacentfaces[i][k]][2] == neighbors[i][j])
                                                ind_faces[ind_faces_count][1] = faces[adjacentfaces[i][k]][2];
					ind_faces_count++;

				}
				//	dprintf("\nbool_hit %d", neighbors[i][j]);
			}
		}
	//}
...
/*
	for (i = 0; i < 10; i++) 
		dprintf("\t%lf",lb_operator[0][i]);
*/
}


void TriMesh::need_gauss_curvatures()
{
//	SymMatrix<double> A(100); // My change
	FILE * pFile;
	int i = 0, j = 0, k = 0;
	point v1, v2, v3;
	double a, b, c, angle_sum = 0;
	int nv = vertices.size();
	gauss_curv.clear(); gauss_curv.resize(nv);
...
//    dprob(A.ncols(), 4, &A, &SymMatrix<double>::MultMv, "SM");
	...
	pFile = fopen ("gauss_curv_file","w");
	for (i = 0; i < nv; i++) {
		
		int n = adjacentfaces[i].size();
		angle_sum = 0;
		for (j = 0; j < n; j++) {
			int vert_indicator = 0;
			for (k = 0; k < 3; k++) {
			if(faces[adjacentfaces[i][j]][k] == i) {
				vert_indicator = k;
				break;
				}
			}
			if (vert_indicator == 0) {
				v1 = vertices[faces[adjacentfaces[i][j]][0]];
				v2 = vertices[faces[adjacentfaces[i][j]][1]];
				v3 = vertices[faces[adjacentfaces[i][j]][2]];
			}
			else if (vert_indicator == 1) {
				v1 = vertices[faces[adjacentfaces[i][j]][1]];
                                v2 = vertices[faces[adjacentfaces[i][j]][0]];
                                v3 = vertices[faces[adjacentfaces[i][j]][2]];
			}
			else {
				v1 = vertices[faces[adjacentfaces[i][j]][2]];
                                v2 = vertices[faces[adjacentfaces[i][j]][0]];
                                v3 = vertices[faces[adjacentfaces[i][j]][1]];
			}
			a = euclid_distance(v1,v2);
			b = euclid_distance(v1,v3);
			c = euclid_distance(v2,v3);
			double arg = acos((a*a + b*b - c*c)/(2*a*b));
			angle_sum += arg;
		}
		gauss_curv[i] = 2*3.14159 - angle_sum;
		if (gauss_curv[i] < 0)  neg_count++;
		//dprintf("\ni = %d naf = %d GAUSS CURVATURE = %lf", i, n, gauss_curv[i]); 
		fprintf (pFile, "%lf\n", gauss_curv[i]);
	}
	dprintf("\n nv %d neg_c %d",nv, neg_count);
	dprintf("\n max %lf min %lf",*(std::max_element(gauss_curv.begin(), gauss_curv.end())), *(std::min_element(gauss_curv.begin(), gauss_curv.end())));
	fclose (pFile);
	dprintf("\nGaussian Curvature Computation End\n");
}
