...
//	return calculate_hausdorff(mesh_A, mesh_B) + sqrt(smooth_diff);
}
...
//	char filename1[100];
	...
	for (int i = 1; i < argc-1; i++) {
		...
		themesh->need_smoothness(); // I added
//		themesh->need_segmentation();
...
//	printf("\nGL1 between the meshes: %f", calculate_gl(meshes[0], meshes[1]));
	...
	fp = fopen(filename1,"a+"); /* open for writing */
