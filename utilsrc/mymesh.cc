...
/*
		meshes[choice-1]->need_normals();
                meshes[choice-1]->need_tstrips();
                meshes[choice-1]->need_bsphere();
                meshes[choice-1]->need_smoothness();
		meshes[choice-1]->need_saliency();
		glutCreateWindow(argv[choice-1]);
        	glClearColor(1.0f, 1.0f, 1.0f, 1.0f); 
        	glutDisplayFunc(redraw);
        	glutMouseFunc(mousebuttonfunc);
        	glutMotionFunc(mousemotionfunc);
...
//	printf("\nHausdorff distance between the meshes: %f", calculate_hausdorff(meshes[0], meshes[1]));
//	printf("\nGL1 between the meshes: %f", calculate_gl(meshes[0], meshes[1]));
/*
	const char *filename1 = argv[3];
	fp = fopen(filename1,"a+");
	
	fprintf(fp,"\n\nGL1\n\n");
     	fprintf(fp,"%s\t%f\n", argv[2],calculate_gl(meshes[0], meshes[1]));

	fprintf(fp,"\n\nHausdorff\n\n");
	fprintf(fp,"%s\t%f\n", argv[2],calculate_hausdorff(meshes[0], meshes[1]));
